...
json_offline_file_path = "Developer/API_APP_VERSION/service-account-credentials.json"

with open(json_file_path) as json_file:
    data = json.load(json_file)
    service_account_info = data['google_service_account']
    LOGIN_PAYLOAD = data['LOGIN_PAYLOAD']
def on_login():
    new_login = login_entry.get()
    new_senha = login_senha.get()
    if new_login:
        LOGIN_PAYLOAD['login'] = new_login
        LOGIN_PAYLOAD['senha'] = new_senha
        status_label.config(text=f"Login atualizado: {new_login}")
    login_window.destroy()
        
if LOGIN_PAYLOAD['login'] == 'a':
    login_window = tk.Tk()
    login_window.title("Login")
    login_window.geometry("250x200")

    label = tk.Label(login_window, text="Digite seu login:")
    label.pack(pady=10)

    login_entry = tk.Entry(login_window)
    login_entry.pack(pady=4)

    label = tk.Label(login_window, text="Digite sua senha:")
    label.pack(pady=4)

    login_senha = tk.Entry(login_window)
    login_senha.pack(pady=4)

    login_button = tk.Button(login_window, text="Realizar Login", command=on_login)
...
def get_access_token_and_id(login_url, payload):
    try:
        ...
        bases = response_data.get("data", {}).get("info", {}).get("base", [])

        base_id = bases[0]["id"] if bases else None
        
        return access_token, user_id, base_id, refresh_token, nome_id
    except requests.exceptions.RequestException as e:
        print(None, "Erro", f"Erro ao fazer login: {str(e)}")
...
INFO_DEFAULT = next(iter(LOCALES.keys()), None)  
...
class MouseCoordinateApp(QWidget):
    ...
    def START_INSERT_EVENT(self):
        if ("pos1" in self.positions and "pos2" in self.positions):
            if (self.combo_box_drive.button.text() != "Click para selecionar o entregador"):
